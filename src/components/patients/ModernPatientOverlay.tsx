...
import { X, Save, User, Phone, Mail, MapPin, Heart, Calendar, Activity, FileText, AlertCircle, Barcode, Edit3, Stethoscope, Building2, Droplets } from 'lucide-react';
        ...
        >
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h1 className="text-lg font-bold text-foreground tracking-tight truncate">{patient.name}</h1>
                    {patient.status && (
                      <Badge variant="outline" className={`${statusColors[patient.status as keyof typeof statusColors]} text-[10px] shrink-0`}>
                        {patient.status}
                      </Badge>
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground font-medium">{patient.patientId} · {patient.department || 'General Medicine'}</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 ml-2">
                <Button variant="outline" size="sm" onClick={() => setInternalEditMode(true)} className="h-8 px-2.5 text-xs">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="rounded-lg border border-border bg-card p-3 text-center">
                <p className="text-2xl font-bold text-foreground">{patient.gender || '—'}</p>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium mt-0.5">Gender</p>
              </div>
            </div>
